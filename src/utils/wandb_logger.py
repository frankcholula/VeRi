...
class WandBLogger:
    def __init__(self, args=None):
        ...
        if self.enabled and not args.evaluate:
            ...
            wandb.init(
                project="VeRi",
                name=f"{args.arch}_{aug_name}_{args.lr:.0e}_{args.train_batch_size}_{args.max_epoch}",
                config=vars(args),
            )
        ...
        if self.enabled and not self.args.evaluate:
            if name is None:
                try:
                    ...
                    aug_name = self._get_aug_name()
                    name = f"{self.args.arch}_{aug_name}_{self.args.lr:.0e}_{self.args.train_batch_size}_{epoch}"
                except:
